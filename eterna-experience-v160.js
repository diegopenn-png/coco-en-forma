...
(function(root){
  ...
  function retiredEnglish(row){var subject=String(row&&row.subject||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();return /(^|[^a-z])ingles([^a-z]|$)/.test(subject)}
  ...
  function buildLearningReportModel(exportData,name){
    ...
    var panels=[
      ...
      {tone:"reinforce",icon:"↗",eyebrow:"VAMOS A REFORZAR",title:"Áreas sugeridas para seguir practicando",text:"Son señales de práctica, no etiquetas permanentes.",items:reinforce.map(function(x){return{label:clean(x.concept_label),detail:(clean(x.subject)||"Aprendizaje")+" · "+countLabel(x.attempts,"intento","intentos"),percent:percent(x.mastery_score)}})},
      {tone:"strategy",icon:"💡",eyebrow:"ASÍ PARECE AYUDARLE MÁS",title:"Estrategias que están funcionando",text:"Eterna seguirá ajustando la forma de explicar según la evidencia acumulada.",items:strategies.map(function(x){var score=Number(x.success_score);return{label:strategyName(x.strategy_key),detail:countLabel(x.evidence_count,"evidencia","evidencias"),percent:isFinite(score)?percent(score):null}})},
