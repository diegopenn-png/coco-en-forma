...
(function(root){
  ...
  function countLabel(value,singular,plural){var n=Number(value||0);return n+" "+(n===1?singular:plural)}
  var SIGNAL_DEFINITION="Una señal es una evidencia orientativa obtenida de una respuesta, un intento, una corrección, una pista utilizada o el resultado de un ejercicio. Varias señales ayudan a estimar el progreso y pueden cambiar con nuevas actividades. No equivalen a una nota escolar, un diagnóstico ni una medición permanente de la capacidad del alumno.";
  ...
  function buildLearningReportModel(exportData,name){
    ...
    var academicMemory=(Array.isArray(exportData.academic_memory)?exportData.academic_memory:[]).filter(function(x){return x&&clean(x.topic_label)});
    var concepts=(Array.isArray(exportData.student_concept_memory)?exportData.student_concept_memory:[]).filter(function(x){return x&&clean(x.concept_label)});
    ...
    var strategies=(Array.isArray(exportData.learning_strategy_memory)?exportData.learning_strategy_memory:[]).filter(function(x){return x&&clean(x.strategy_key)&&Number(x.evidence_count||0)>=2}).sort(function(a,b){var ds=Number(b.success_score||0)-Number(a.success_score||0);return Math.abs(ds)>.001?ds:Number(b.evidence_count||0)-Number(a.evidence_count||0)}).slice(0,3);
    var profile=exportData.student_profile||{},subjects=uniqueStrings(observed.map(function(x){return x.subject}).concat(academicMemory.map(function(x){return x.subject}))),attempts=observed.reduce(function(sum,x){return sum+Number(x.attempts||0)},0),errors=observed.reduce(function(sum,x){return sum+Number(x.errors||0)},0),partials=observed.reduce(function(sum,x){return sum+Number(x.partials||0)},0),independent=observed.reduce(function(sum,x){return sum+Number(x.independent_successes||0)},0),assisted=observed.reduce(function(sum,x){return sum+Number(x.assisted_successes||0)},0),activeDays=recentUsageDays(exportData.usage||[]);
    ...
    var bars=evaluable.slice().sort(function(a,b){return Number(b.attempts||0)-Number(a.attempts||0)}).slice(0,6).map(function(x){return{label:clean(x.concept_label),sublabel:clean(x.subject)||"Aprendizaje",value:percent(x.mastery_score),detail:countLabel(x.attempts,"interacción","interacciones")}}),panels=[];
    if(strongest.length)panels.push({tone:"strength",icon:"★",eyebrow:"LO QUE MEJOR ENTIENDE",title:"Aprendizajes más asentados",text:"Solo aparecen cuando ya hay práctica repetida.",items:strongest.map(function(x){return{label:clean(x.concept_label),detail:clean(x.subject)||"Aprendizaje",percent:percent(x.mastery_score)}})});
    if(reinforce.length)panels.push({tone:"reinforce",icon:"↗",eyebrow:"PARA SEGUIR PRACTICANDO",title:"Aprendizajes todavía en proceso",text:"No son etiquetas: pueden cambiar con la práctica.",items:reinforce.map(function(x){return{label:clean(x.concept_label),detail:(clean(x.subject)||"Aprendizaje")+" · "+countLabel(x.attempts,"interacción","interacciones"),percent:percent(x.mastery_score)}})});
    if(strategies.length)panels.push({tone:"strategy",icon:"💡",eyebrow:"CÓMO LE AYUDA ETERNA",title:"Formas de explicar que le están ayudando",text:"Solo se muestran estrategias observadas más de una vez.",items:strategies.map(function(x){var score=Number(x.success_score);return{label:strategyName(x.strategy_key),detail:countLabel(x.evidence_count,"observación","observaciones"),percent:isFinite(score)?percent(score):null}})});
