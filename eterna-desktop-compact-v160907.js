/* ETERNA desktop compact header · 160.93.21
 * Scope: desktop-only compact mode/action bar + verified Coco JPEG mascot.
 * Keeps mobile/tablet unchanged.
 */
(function(){
  'use strict';
  if(window.__ETERNA_DESKTOP_COMPACT_1609321__)return;
  window.__ETERNA_DESKTOP_COMPACT_1609321__=true;
  ...
  function ensureMascot(){
    ...
    if(!mascot){
      ...
      mascot.innerHTML='<img src="./coco-flex-eterna.jpg?v=1609424" alt="">';
      bar.insertBefore(mascot,bar.firstChild);
    }
  }

  var style=document.createElement('style');
  style.id='eterna-desktop-compact-v1609321';
  style.textContent='@media (min-width:761px){'+
    ...
    '#eternaOverlayV159 .eternaV160CocoFlex img{display:block!important;width:112px!important;height:112px!important;object-fit:cover!important;object-position:center 36%!important;border-radius:28px!important;filter:drop-shadow(0 4px 7px rgba(23,63,89,.10));-webkit-mask-image:radial-gradient(ellipse at center,#000 58%,rgba(0,0,0,.78) 70%,transparent 92%);mask-image:radial-gradient(ellipse at center,#000 58%,rgba(0,0,0,.78) 70%,transparent 92%);animation:eternaCocoFlex1609321 1.8s ease-in-out infinite!important;transform-origin:50% 82%!important;}'+
  ...
  '}'+
  '@keyframes eternaCocoFlex1609321{0%,100%{transform:translateY(2px) rotate(-1deg) scale(1)}35%{transform:translateY(-3px) rotate(1deg) scale(1.035)}70%{transform:translateY(0) rotate(-.5deg) scale(1.015)}}'+
