(function (root) {
  ...
  var C = root.CocoV144;
  if (!C || root.CocoPadelV146) return;
  ...
  function standingsTableHtml(rows, scoring) {
    return '<div class="c144TableWrap"><table class="c144Table"><thead><tr><th>Pos.</th><th>Jugador</th><th>PJ</th><th>PG</th><th>PE</th><th>PP</th><th>GG</th><th>GP</th><th>DG</th><th>PTS</th></tr></thead><tbody>' + (rows.length ? rows.map(function (row) { return '<tr><td class="rank">' + row.position + '</td><td><b>' + C.esc(row.name) + '</b><br><small>' + C.esc(row.code) + '</small></td><td>' + row.played + '</td><td>' + row.won + '</td><td>' + row.drawn + '</td><td>' + row.lost + '</td><td>' + row.gamesWon + '</td><td>' + row.gamesLost + '</td><td>' + (row.gameDifference > 0 ? "+" : "") + row.gameDifference + '</td><td><b>' + row.points + '</b></td></tr>'; }).join("") : '<tr><td colspan="10">Todavía no hay resultados completos.</td></tr>') + '</tbody></table></div><p class="c144Notice">Regla: ' + (scoring.mode === "games" ? "los puntos equivalen a los games ganados" : scoring.win + " por victoria, " + scoring.draw + " por empate y " + scoring.loss + " por derrota") + '. Desempates: puntos, diferencia de games, games ganados y resultado directo.</p>';
  }
  ...
  function sessionDetailHtml(selected, championship) {
    ...
    return '<section class="c144Card c144PadelPanel c146SessionDetail"><div class="c146DetailBack"><button type="button" class="c144Secondary" data-padel146-close-session>‹ Volver al campeonato</button></div><div class="c144PadelPanelHead"><div><span class="c144Eyebrow">' + (selected.championshipId ? "JORNADA DEL CAMPEONATO" : "SESIÓN LIBRE") + '</span><h3>' + C.esc(selected.name) + '</h3><p>' + C.esc(selected.date) + ' · ' + selected.participants.length + ' jugadores · ' + selected.courts + ' pistas</p></div><span class="c144Chip">' + done + '/' + selected.matches.length + ' guardados</span></div><div class="c146SessionState"><span>' + (selected.matches.length - done) + ' partidos pendientes</span><span>' + done + ' resultados guardados</span></div><div class="c144Actions"><button type="button" class="c144Primary" data-padel144-save-all>Guardar toda la jornada</button></div><div class="c144Rounds">' + Object.keys(rounds).map(function (round) { return '<section class="c144Round"><header><b>Ronda ' + round + '</b><span>' + (selected.timerMode === "unlimited" ? "Sin límite" : selected.matchMinutes + " min") + '</span></header><div class="c144Matches">' + rounds[round].map(function (match) { return matchHtml(selected, match); }).join("") + '</div></section>'; }).join("") + '</div><div class="c144StandingsTools"><h4>Resumen de esta jornada</h4></div>' + standingsTableHtml(rows, scoring) + '</section>';
  }
  ...
  function render() {
    ...
    C.setModalTitle("Coco Pádel", "HERRAMIENTA ILIMITADA · v146.0");
  ...
  async function open() {
    C.openModal({ module: "padel", title: "Coco Pádel", kicker: "HERRAMIENTA ILIMITADA · v146.0", html: '<div class="c144Empty"><b>Coco está cargando el club…</b></div>', dispose: dispose });
    ...
    view = "mixing"; currentSessionId = null; selectedPlayerId = null; mixingDraft = null; mixingSession = null; mixingScreen = "home"; mixingStep = 1; championshipScreen = "home"; championshipCreateStep = 1; championshipDraft = null; archivedVisible = false; dateFormOpen = false; participantsOpen = false; draft = null; render();
  }

  function dispose() { if (controller) controller.abort(); controller = null; draft = null; mixingDraft = null; mixingSession = null; currentSessionId = null; championshipDraft = null; dateFormOpen = false; participantsOpen = false; busy = false; }

  var api = {
    version: "146.0.0", open: open, normalize: normalizeState, blankState: blankState, createPlayer: createPlayer,
