(function (root) {
  ...
  var C = root.CocoV144;
  if (!C || root.CocoPadelV151) return;
  ...
  function standingsPayload(championship, targetState) {
    ...
    var headers = ["Posición", "Jugador", "Código", "PJ", "PG", "PP", "Sets ganados", "Sets perdidos", "Dif. sets", "Games ganados", "Games perdidos", "Dif. games", "Puntos"];
  ...
  function playersPayload(targetState) {
    targetState = targetState || state;
    var headers = ["Jugador", "Código", "Nivel", "Estado", "Campeonatos", "Jornadas", "PJ", "PG", "PP", "Sets ganados", "Sets perdidos", "Games ganados", "Games perdidos", "Puntos"];
    ...
    var text = "COCO PÁDEL\nJUGADORES\n\n" + (targetState.players.length ? targetState.players.map(function (player) { var stats = playerStats(targetState, player.id); return player.name + " (" + player.code + ") · " + levelLabel(player.currentLevel) + " · " + (player.active ? "Activo" : "Inactivo") + " · " + stats.played + " PJ · " + stats.won + " PG · " + stats.points + " pts"; }).join("\n") : "Aún no hay jugadores.");
  ...
  function standingsTableHtml(rows, scoring) {
    ...
    var cards = rows.length ? '<div class="c149RankingCards c151RankingCards">' + rows.map(function (row) {
      return '<article><div class="c149RankIdentity"><strong>#' + row.position + '</strong><div><b>' + C.esc(row.name) + '</b><span>' + C.esc(row.code) + '</span></div><em>' + row.points + ' pts</em></div><div class="c149RankMetrics"><span><b>' + row.played + '</b>PJ</span><span><b>' + row.won + '</b>PG</span><span><b>' + row.lost + '</b>PP</span><span><b>' + row.setsWon + '</b>SG</span><span><b>' + row.setsLost + '</b>SP</span><span><b>' + (row.setDifference > 0 ? "+" : "") + row.setDifference + '</b>DS</span><span><b>' + row.gamesWon + '</b>GG</span><span><b>' + row.gamesLost + '</b>GP</span><span><b>' + (row.gameDifference > 0 ? "+" : "") + row.gameDifference + '</b>DG</span></div></article>';
    }).join("") + '</div>' : empty;
    var table = rows.length ? '<div class="c144TableWrap c149RankingTableDesktop"><table class="c144Table c151RankingTable"><thead><tr><th>Pos.</th><th>Jugador</th><th>PJ</th><th>PG</th><th>PP</th><th>SG</th><th>SP</th><th>DS</th><th>GG</th><th>GP</th><th>DG</th><th>PTS</th></tr></thead><tbody>' + rows.map(function (row) {
  ...
  async function open() {
    C.openModal({ module: "padel", title: "Coco Pádel", kicker: "HERRAMIENTA ILIMITADA · v151.0", html: '<div class="c144Empty"><b>Coco está cargando el club…</b></div>', dispose: dispose });
  ...
  var api = {
    version: "151.0.0", open: open, normalize: normalizeState, blankState: blankState, createPlayer: createPlayer,
    ...
    audit: function (targetState) {
      ...
      return {
        ...
        resultEntry: "set-by-set", maxSetsVisible: 3, rankingsIncludeSets: true, rankingsRecalculateFromResults: true,
