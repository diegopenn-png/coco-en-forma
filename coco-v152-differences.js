(function (root) {
  ...
  async function renderIntro() {
    ...
    if (!body) return;
    C.setModalTitle("Encuentra las diferencias", "ATENCIÓN VISUAL · v152.0");
  ...
  function sceneSource(side) {
    if (side === "left") return "scenes/scene-" + game.scene.id + "-v152-left.webp?v=1520";
    return "scenes/scene-" + game.scene.id + "-v152-right-" + (game.variantIndex + 1) + "-l" + levelSelected + ".webp?v=1520";
  }

  function sceneMarkup(side) {
    var src = sceneSource(side);
    ...
    images.forEach(function (image) {
      if (image.complete && image.naturalWidth) ready(image);
        ...
        image.onerror = function () {
          ...
          var body = C.body();
          if (!body) return;
          body.innerHTML = '<div class="c144Empty"><b>No se pudo cargar el escenario</b><span>Comprueba que el ZIP incluya las escenas v152.</span><button type="button" class="c144Secondary" data-diff151-retry>Reintentar</button></div>';
  ...
  async function open() {
    C.openModal({ module: "differences", title: "Encuentra las diferencias", kicker: "ATENCIÓN VISUAL · v152.0", html: '<div class="c144Empty"><b>Coco está preparando el escenario…</b></div>', dispose: dispose });
  ...
  var api = {
    ...
    audit: function () {
      return {
        ...
        precomposedPairs: true, levelSpecificPairs: true, noExtraUnclickableDifferences: true,
        ...
        renderer: "paired-level-specific-scenes-v152"
      };
    }
  };
