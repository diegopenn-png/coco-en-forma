/* Coco en Forma · Service Worker v160 FINAL4.28 · core-first */
const CACHE_VERSION="coco-en-forma-v160.0.0-final4.28";
...
const CORE=[
  ...
  "./eterna-v159.js","./eterna-v159.css","./eterna-experience-v160.js?v=16068","./share/eterna.png",
...
self.addEventListener("activate",e=>{e.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith(CACHE_PREFIX)&&k!==CACHE_VERSION).map(k=>caches.delete(k)));if(self.registration.navigationPreload)try{await self.registration.navigationPreload.enable()}catch(_e){}await self.clients.claim()})())});
self.addEventListener("message",e=>{if(e.data&&e.data.type==="SKIP_WAITING")self.skipWaiting()});
async function networkFirst(e){try{const preload=e.request.mode==="navigate"&&e.preloadResponse?await e.preloadResponse:null,r=preload||await fetch(e.request);if(r&&r.ok)e.waitUntil(caches.open(CACHE_VERSION).then(c=>c.put(e.request,r.clone())));return r}catch(_e){return(await caches.match(e.request,{ignoreSearch:false}))||(await caches.match(e.request,{ignoreSearch:true}))||(await caches.match(INDEX_URL))}}
...
self.addEventListener("fetch",e=>{if(e.request.method!=="GET")return;const u=new URL(e.request.url);if(u.origin!==self.location.origin)return;const doc=e.request.mode==="navigate"||e.request.destination==="document",bootstrap=/\/(coco-v153-fixes\.js|coco-v155-identity\.js)$/.test(u.pathname);if(doc||bootstrap){e.respondWith(networkFirst(e));return}e.respondWith(stale(e))});
