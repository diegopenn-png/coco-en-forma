...
test("an explicit topic change wins over a pending answer phase", () => {
  ...
  assert.match(client, /target\.topic,target\.subject,target\.concept/);
  assert.match(client, /function standaloneSchoolSubject\(raw\)/);
  assert.match(client, /explicitNewTopic\|\|standaloneSubject/);
});
