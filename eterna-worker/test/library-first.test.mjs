...
test('reviewed library preserves the original 160 objects except explicit audited corrections',async()=>{const h=harness();assert.equal(h.sandbox.ETERNA_LIBRARY_CONTENT.release_id,"eterna-library-2026.09-v7-370-c26c61c");const{createHash}=await import('node:crypto');assert.equal(createHash('sha256').update(JSON.stringify(restoreReviewedOriginals(h.lessons).slice(0,160))).digest('hex'),"208b13a31252598c22dd5b2dfa41ce9a4fc571ff3cefa0e82abd28358c109da1");});
