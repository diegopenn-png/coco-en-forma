...
test('library has 370 actual micro-lessons, 1110 distinct structured checks and transparent provenance',()=>{
  const h=harness();assert.equal(h.lessons.length,370);assert.equal(new Set(h.lessons.map(l=>l.id)).size,370);assert.equal(h.lessons.flatMap(l=>l.quiz).length,1110);
...
test('all 1110 known checks rederive correct and wrong answers from trusted content, never from client keys',()=>{
...
test('v2 natural request grammar matches the whole intent, including polite speech',()=>{
 const h=harness();let n=0;for(const l of h.lessons)for(const t of [`¿Me puedes explicar ${l.title}?`,`Hola Eterna, me podrías explicar ${l.title}, por favor`,`Necesito ayuda con ${l.title}`,`Podemos repasar ${l.title}`,`Explícame ${l.title} paso a paso`]){assert.equal(h.library.exactLesson(t,profile(l))?.id,l.id,t);n++}assert.equal(n,1850)
});
...
test('reviewed library preserves the original 160 objects except explicit audited corrections',async()=>{const h=harness();assert.equal(h.sandbox.ETERNA_LIBRARY_CONTENT.release_id,"eterna-library-2026.09-v7-370-content-c26c61c");const{createHash}=await import('node:crypto');assert.equal(createHash('sha256').update(JSON.stringify(restoreReviewedOriginals(h.lessons).slice(0,160))).digest('hex'),"208b13a31252598c22dd5b2dfa41ce9a4fc571ff3cefa0e82abd28358c109da1");});
