...
test('library has 410 actual micro-lessons, 1230 distinct structured checks and transparent provenance',()=>{
  const h=harness();assert.equal(h.lessons.length,410);assert.equal(new Set(h.lessons.map(l=>l.id)).size,410);assert.equal(h.lessons.flatMap(l=>l.quiz).length,1230);
...
test('all 1230 known checks rederive correct and wrong answers from trusted content, never from client keys',()=>{
...
test('v2 natural request grammar matches the whole intent, including polite speech',()=>{
 const h=harness();let n=0;for(const l of h.lessons)for(const t of [`¿Me puedes explicar ${l.title}?`,`Hola Eterna, me podrías explicar ${l.title}, por favor`,`Necesito ayuda con ${l.title}`,`Podemos repasar ${l.title}`,`Explícame ${l.title} paso a paso`]){assert.equal(h.library.exactLesson(t,profile(l))?.id,l.id,t);n++}assert.equal(n,2050)
});
...
test('reviewed library preserves the original 160 objects except explicit audited corrections',async()=>{const h=harness();assert.equal(h.sandbox.ETERNA_LIBRARY_CONTENT.release_id,"eterna-library-2026.09-v8-410-consolidated-1c484e");const{createHash}=await import('node:crypto');assert.equal(createHash('sha256').update(JSON.stringify(restoreReviewedOriginals(h.lessons).slice(0,160))).digest('hex'),"208b13a31252598c22dd5b2dfa41ce9a4fc571ff3cefa0e82abd28358c109da1");});
...
function restoreReviewedOriginals(current){
 const m=JSON.parse(text('../../qa/curriculum-review/corrections.json')),items=undoPanelChanges(plain(current.slice(0,217))),by=new Map(items.map(l=>[l.id,l]));
