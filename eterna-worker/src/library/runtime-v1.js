...
(function(root){
  ...
  function cordial(kind,{text='',profile={},base={},pedState={},now=new Date()}={}){
    ...
    const choose=variants=>variants[index%variants.length];
    ...
    switch(kind){
      ...
      case'how_are_you':return choose([`Estoy bien y lista para ayudarte${name}. ${pending?'Podemos retomar la pregunta cuando quieras.':'¿Vienes con una duda o te apetece practicar?'}`,`Todo bien por aquí${name}. ${pending?'Seguimos desde donde lo dejamos, sin empezar de cero.':'Cuéntame qué necesitas del cole.'}`,`Lista para echarte una mano${name}. ${pending?'La pregunta sigue pendiente; vamos con calma.':'¿Qué tienes entre manos hoy?'}`]);
      ...
      case'goodbye':return choose([`¡Hasta luego${name}! Descansa y disfruta también fuera de la pantalla.`,`Nos vemos${name}. Buen trabajo por hoy; ahora toca desconectar un poco.`,`Hasta la próxima${name}. Cuando vuelvas, seguimos desde aquí.`]);
      ...
      case'mission':return choose(['Estoy para ayudarte a entender y avanzar por ti mismo. Puedo explicarte algo de otra forma, darte una pista, revisar un intento o practicar una pregunta cada vez.','Puedo acompañarte con deberes, dudas, repasos y exámenes. La idea es que lo comprendas tú, no hacer pasar mis respuestas por tu trabajo.','Soy tu apoyo para el cole: explicamos, practicamos o revisamos paso a paso, siempre dejando que el trabajo final sea tuyo.']);
      ...
      case'tired':return choose([`Gracias por decírmelo${name}. Hagamos una pausa fuera de la pantalla. Después volvemos con un solo paso pequeño; no hay que hacerlo todo de golpe.`,`Te entiendo${name}. Para un momento, muévete un poco y descansa la vista. Luego elegimos solo una cosa pequeña.`,`Vale${name}, no hace falta forzar. Dejamos aquí una pausa y, cuando vuelvas, retomamos por el paso más sencillo.`]);
      ...
      case'exam_nerves':return choose([`Entiendo que el examen pueda preocuparte${name}. No necesitas demostrarlo todo de golpe: practicamos una pregunta y vemos qué te ayuda. Si los nervios te desbordan, coméntalo con tu familia o tu profesor.`,`Es normal llegar con nervios${name}. Vamos a convertirlos en un plan: una pregunta, una corrección y el siguiente paso. Si la preocupación se hace muy grande, pide apoyo a un adulto de confianza.`,`Vamos con calma${name}. No hace falta pensar en todo el examen ahora; empezamos por una pregunta concreta y ajustamos desde ahí.`]);
      ...
      case"overloaded":return choose(["Cuando se juntan muchas tareas puede agobiar. Separamos lo urgente de lo que puede esperar y elegimos un primer paso pequeño. ¿Qué tienes que entregar antes?","Vaya, son muchas cosas a la vez. Hagamos una lista corta y empecemos solo por lo que vence primero. ¿Qué entrega es la más cercana?","No intentemos abarcarlo todo de golpe. Elige la tarea más urgente y la dividimos en un primer paso que puedas hacer ahora."]);
      case"frustrated":return choose(["Es frustrante cuando lo intentas y no sale. No hace falta repetirlo igual: revisamos un solo paso o cambiamos de ejemplo. Tu valor no depende de este ejercicio.","Te entiendo; atascarse después de intentarlo cansa. Vamos a localizar el primer paso que no encaja y probamos otra estrategia.","Vale, cambiemos el enfoque. Podemos hacer una pausa breve o mirar un ejemplo más pequeño antes de volver al ejercicio."]);
      case"celebrate":return choose(["¡Qué bien que notes tu avance! Fíjate en qué te ayudó esta vez: esa estrategia puede servirte de nuevo.","¡Bien hecho! Más que la nota, interesa descubrir qué paso te funcionó para poder repetirlo.","Se nota el avance. Guarda la estrategia que usaste hoy: puede ayudarte cuando aparezca un ejercicio parecido."]);
      case"show_reasoning":return choose(["Buen objetivo. Conectamos la idea con algo que ya sabes, vemos por qué funciona y la comprobamos con un ejemplo. ¿Qué parte quieres entender primero?","Perfecto: vamos a buscar el porqué, no una frase para memorizar. Dime dónde deja de tener sentido para ti.","Eso me gusta. Construimos el razonamiento paso a paso y luego lo probamos en un caso nuevo. ¿Por dónde empezamos?"]);
      case"teach_back":return choose(["Claro. Explícalo con tus palabras; primero miraré qué idea has captado y después qué conviene ajustar.","Adelante: cuéntamelo como se lo explicarías a un compañero. Después afinamos solo lo que haga falta.","Te escucho. No hace falta repetir mis palabras: usa las tuyas y vemos juntos si la idea queda completa."]);
      ...
      case"integrity":return choose(["Puedo ayudarte a que sepas hacerlo, pero no a presentar mi trabajo como tuyo. Muéstrame una pregunta y lo que has intentado; empezamos con una pista.","No voy a hacer una entrega para que parezca tuya, pero sí puedo ayudarte de verdad: enséñame el primer ejercicio y tu intento.","Vamos a conseguir que puedas resolverlo tú. Te doy una pista cada vez y revisamos tus pasos, sin convertir mi respuesta en algo para copiar."]);
      case"disagreement":return choose(["Gracias por señalarlo. Puedo equivocarme. Dime qué paso o afirmación no encaja y lo revisamos sin dar por hecho que yo tenga razón.","Puede que tengas razón; comprobémoslo. Señala el paso que ves mal y lo contrastamos con el enunciado o con otra fuente.","Bien visto al cuestionarlo. No voy a defender mi respuesta por inercia: dime dónde aparece la discrepancia y lo revisamos."]);
      ...
      case"curiosity":return choose(["La curiosidad es un buen punto de partida. Elegimos un tema del cole y lo exploramos con una pregunta concreta. ¿Qué te gustaría entender?","Genial. Trae esa curiosidad a una pregunta concreta y tiramos del hilo paso a paso.","Eso da para aprender mucho. ¿Qué tema del cole te apetece investigar primero?"]);
      ...
      case"topic_choice":return choose(["Claro, puedes cambiar de tema o hacer otra pregunta. Dime cuál; si luego quieres volver, retomamos el contexto que tengamos.","Sí, cambiamos sin problema. ¿Qué tema escolar quieres ver ahora?","Por supuesto. Dejamos esto en pausa y vamos al tema que elijas. ¿Cuál es?"]);
