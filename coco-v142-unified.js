(function () {
  ...
  function celebrationStorageKey(completionKey) {
    return "coco_v142_celebrated_" + encodeURIComponent(String(completionKey));
  }

  function celebrationWasStored(completionKey) {
    try { return localStorage.getItem(celebrationStorageKey(completionKey)) === "1"; }
    catch { return false; }
  }

  function storeCelebration(completionKey) {
    try { localStorage.setItem(celebrationStorageKey(completionKey), "1"); }
    catch {}
  }
  ...
  function stopVictoryMusic() {
    ...
    victoryTimer = 0;
    victoryNodes.forEach(function (node) {
  ...
  function playVictoryMusic() {
    try { if (localStorage.getItem("coco_sonido") === "0") return false; }
    catch {}
    try {
      var Context = window.AudioContext || window.webkitAudioContext;
      if (!Context) return false;
      ...
      master.gain.exponentialRampToValueAtTime(.52, start + .035);
      master.gain.setValueAtTime(.52, start + 1.18);
      master.gain.exponentialRampToValueAtTime(.0001, start + 1.48);
      master.connect(audioContext.destination);
      victoryNodes.push(master);
      /* Acorde ascendente original y breve; no usa archivos ni melodías comerciales. */
      [
        { frequency: 523.25, at: 0, length: .42 },
        { frequency: 659.25, at: .16, length: .42 },
        { frequency: 783.99, at: .32, length: .5 },
        { frequency: 659.25, at: .57, length: .4 },
        { frequency: 1046.5, at: .78, length: .66 }
        ...
        gain.gain.setValueAtTime(.0001, at);
        gain.gain.exponentialRampToValueAtTime(index === 4 ? .13 : .09, at + .025);
        gain.gain.exponentialRampToValueAtTime(.0001, at + note.length);
        oscillator.connect(gain);
        gain.connect(master);
        oscillator.start(at);
        oscillator.stop(at + note.length + .03);
        victoryNodes.push(oscillator, gain);
      });
      auditState.victoryMusicRuns++;
      auditState.victoryMusicDurationMs = 1500;
      victoryTimer = setTimeout(stopVictoryMusic, 1600);
      return true;
    } catch {
      stopVictoryMusic();
      return false;
  ...
  function completionCelebration(completionKey) {
    if (!completionKey || celebratedCompletions[completionKey] || celebrationWasStored(completionKey)) return false;
    var now = Date.now();
    celebratedCompletions[completionKey] = true;
    storeCelebration(completionKey);
    cancelLegacyEffects();
    cancelConfetti();
    stopVictoryMusic();
    auditState.celebrationRuns++;
    playVictoryMusic();
  ...
  function celebrate(gameId, instanceKey, userId) {
    gameId = safeGameId(gameId);
    if (gameId !== "padel" || !text(instanceKey)) return false;
    var daily = window.CocoDailyV134;
    var resolvedUser = userId || (daily && typeof daily.userId === "function" ? daily.userId() : "") || "visitante";
    var completionKey = String(resolvedUser) + "|padel|" + text(instanceKey);
    auditState.completions.padel = (auditState.completions.padel || 0) + 1;
    return completionCelebration(completionKey);
  }
  ...
  window.CocoResponseV142 = Object.freeze({
    show: show,
    sound: sound,
  ...
  window.CocoResponseV141 = window.CocoResponseV142;
  window.CocoResponseV135 = window.CocoResponseV142;
  ...
  window.CocoQualityV142 = Object.freeze({
    version: VERSION,
    ...
    audit: function () {
      ...
      return {
        ...
        celebrationRuns: auditState.celebrationRuns,
        victoryMusicRuns: auditState.victoryMusicRuns,
        victoryMusicDurationMs: auditState.victoryMusicDurationMs,
        victoryAudioNodes: victoryNodes.length,
        completions: Object.assign({}, auditState.completions)
      };
    }
  });
  window.CocoQualityV135 = window.CocoQualityV142;
})();
