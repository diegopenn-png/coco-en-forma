...
(function(root){
  ...
  function retiredEnglishCopy(v){var s=String(v||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase();return /(^|[^a-z])ingles([^a-z]|$)/.test(s)}
  ...
  function reportBodyHtml(model){
    ...
    var summaryHtml=model.summary?'<p class="cocoV160100PlainSummary">'+escapeReportHtml(model.summary)+'</p>':"";
    ...
    var signalHtml=model.signalDefinition?'<section class="cocoV16083SignalNote"><b>¿Qué significa “señal observada”?</b><p>'+escapeReportHtml(model.signalDefinition)+'</p></section>':"";
  ...
  function injectV16084Styles(){
    ...
    var style=document.createElement("style");style.id="coco-v16084-report-kit-css";style.textContent=reportKitCss(false)+
      ...
      "#cocoApp .cocoMiniJuego[role='button']:focus-visible{outline:3px solid #2FA9DC!important;outline-offset:3px!important}"+
  ...
  var reportKitV16084=Object.freeze({version:"160.100-family-profile-reports",coreHtml:reportCoreHtml,documentHtml:reportDocumentHtml,signature:modelSignature,export:exportReportModel,share:shareReportModel,placeExport:placeReportExportAction,captureLearning:captureLearningReportModel,familyDocumentHtml:familyReportDocumentHtml,familyHtml:familyReportHtml});root.CocoFamilyReportKitV16084=reportKitV16084;root.CocoFamilyReportKitV16083=reportKitV16084;

  function numberFromText(v){var m=String(v||"").replace(/\./g,"").replace(",",".").match(/-?\d+(?:\.\d+)?/);return m?Number(m[0]):0}
  function cocoSkillCopy(label){
    var key=normalizeText(label);
    if(key.indexOf("cuantitativo")>=0)return{label:"Cálculo y patrones",detail:"Números, cálculo y series"};
    if(key==="memoria")return{label:"Memoria",detail:"Recordar y mantener información"};
    if(key.indexOf("atencion")>=0||key.indexOf("ejecutivo")>=0)return{label:"Atención y estrategia",detail:"Concentrarse, decidir y cambiar de estrategia"};
    if(key.indexOf("velocidad")>=0)return{label:"Rapidez con precisión",detail:"Responder con agilidad y cuidado"};
    if(key.indexOf("lenguaje")>=0)return{label:"Lenguaje",detail:"Comprender palabras y aprender"};
    if(key.indexOf("percepcion")>=0||key.indexOf("visomotora")>=0)return{label:"Percepción y coordinación",detail:"Coordinar la vista y la respuesta"};
    return{label:String(label||"Habilidad"),detail:"Habilidad practicada en Coco"}
  }
  function spanishList(values){
    values=(values||[]).filter(Boolean);if(values.length<2)return values[0]||"";if(values.length===2)return values[0]+" y "+values[1];return values.slice(0,-1).join(", ")+" y "+values[values.length-1]
  }
  function buildGamesReportModel(modal,card){
    if(!modal)return null;var hero=modal.querySelector(".cocoFamilyHero"),stats=modal.querySelector(".cocoFamilyStats"),barRoot=modal.querySelector(".cocoFamilyBars");if(!hero||!barRoot)return null;
    ...
    var bars=Array.prototype.slice.call(barRoot.querySelectorAll(":scope > article,article")).map(function(x){var b=x.querySelector("b"),strong=x.querySelector("strong"),copy=cocoSkillCopy(b?String(b.textContent||"").trim():"Habilidad"),evidence=Math.max(0,Number(x.dataset.cocoEvidence||0));return{label:copy.label,sublabel:copy.detail,value:strong?numberFromText(strong.textContent):0,evidence:evidence}}).filter(function(x){return x.evidence>0&&!retiredEnglishCopy(x.label)});
    var assessable=bars.filter(function(x){return x.evidence>=2}),ordered=assessable.slice().sort(function(a,b){return b.value-a.value}),best=ordered[0]||null,lowest=ordered.length>1?ordered[ordered.length-1]:null,names=bars.map(function(x){return x.label.toLowerCase()}),summary=bars.length?"Con Coco ha entrenado "+spanishList(names)+".":"Todavía no hay partidas registradas para explicar qué habilidades ha entrenado.";
    summary+=best?" Las comparaciones solo usan habilidades practicadas al menos dos veces.":bars.length?" Aún hace falta repetir alguna habilidad antes de destacar fortalezas o aspectos para practicar.":"";
    var panels=[];
    if(best)panels.push({tone:"strength",icon:"★",eyebrow:"LO QUE MEJOR LE HA IDO",title:"Habilidad más asentada",items:[{label:best.label,detail:best.sublabel,percent:best.value}],text:"Es el mejor resultado entre las habilidades que ha practicado al menos dos veces."});
    if(lowest)panels.push({tone:"reinforce",icon:"↗",eyebrow:"PARA SEGUIR PRACTICANDO",title:"Habilidad todavía en proceso",items:[{label:lowest.label,detail:lowest.sublabel,percent:lowest.value}],text:"No es una etiqueta: el resultado puede cambiar con nuevas partidas."});
    var nextStep=lowest?{eyebrow:"QUÉ RECOMENDAMOS AHORA",title:"Seguir variando los juegos",text:"Puede alternar "+best.label.toLowerCase()+" con "+lowest.label.toLowerCase()+" en días distintos."}:bars.length?{eyebrow:"QUÉ RECOMENDAMOS AHORA",title:"Repetir con calma",text:"Conviene practicar estas habilidades en varios días antes de comparar resultados."}:null;
    return{theme:"games",eyebrow:"JUEGOS · COCO",title:"Habilidades entrenadas con Coco",subtitle:"Un resumen sencillo de lo que realmente ha practicado en los juegos.",summary:summary,personName:name,personMeta:age,hero:{eyebrow:best?"MEJOR RESULTADO OBSERVADO":"SIN VALORACIÓN TODAVÍA",title:best?"Se desenvuelve mejor en "+best.label:"Necesitamos un poco más de práctica",text:best?"Esta lectura se basa en práctica repetida y puede cambiar con nuevas partidas.":"Una sola partida no basta para decir qué habilidad se le da mejor o cuál necesita reforzar.",percent:best?best.value:null},metrics:metrics,bars:bars,barEyebrow:"HABILIDADES PRACTICADAS",barTitle:"Lo que ha entrenado",barScale:"Resultado orientativo · 0–100",panels:panels,groups:[],nextStep:nextStep,note:"Informe orientativo. Resume únicamente la actividad registrada en los juegos de Coco; no mide inteligencia, no es una prueba clínica y no establece diagnósticos."}
  }
  ...
  function buildFamilyReportModel(modal,card,incoming){
    ...
    return{kind:"family",title:"Así está avanzando",subtitle:"Lo que ha trabajado con Eterna y las habilidades que ha entrenado con Coco, explicado de forma sencilla.",generatedAt:new Date().toISOString(),student:{name:name,age:games&&games.personMeta||"",schoolYear:learningMeta.schoolYear,ccaa:learningMeta.ccaa},learning:learning,games:games,learningStatus:familyReportState.learningStatus,gamesStatus:familyReportState.gamesStatus}
  }

  function familyChapterHtml(model,kind,status){
    var learning=kind==="learning",eyebrow=learning?"APRENDIZAJE · ETERNA":"JUEGOS · COCO",title=learning?"Aprendizaje con Eterna":"Habilidades entrenadas con Coco",subtitle=model&&model.subtitle||(learning?"Solo aparecen las materias y los conceptos que realmente ha trabajado.":"Solo aparecen las habilidades que realmente ha practicado en los juegos.");
  ...
  function familyReportDocumentHtml(family){
    ...
    return '<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Así está avanzando · Coco en Forma</title><style>body{margin:0;background:#edf6fa;color:#173f59}'+reportKitCss(true)+'.cocoV16083Document>footer{margin-top:18px;padding-top:13px;border-top:1px solid #dceaf0;color:#718692;font-size:10px;line-height:1.45}</style></head><body data-coco-return-url="'+escapeReportHtml(returnUrl)+'"><main class="cocoV16083Document"><div class="cocoV16083DocTop"><div><b>COCO EN FORMA · ZONA FAMILIAR</b><strong class="cocoV16086DocumentTitle">Así está avanzando</strong><span>'+escapeReportHtml(reportDate())+'</span></div><div>'+familyStudentHtml(family.student)+'<div class="cocoV16085DocActions"><button type="button" class="is-back" onclick="try{var u=document.body.getAttribute(\'data-coco-return-url\')||\'/\';if(window.opener&&!window.opener.closed){window.opener.focus();window.close()}else{location.href=u}}catch(e){location.href=document.body.getAttribute(\'data-coco-return-url\')||\'/\'}">← Volver</button><button type="button" class="is-print" onclick="window.print()">Imprimir o guardar como PDF</button></div></div></div>'+familyReportHtml(family,false)+'<footer>Informe orientativo. Aprendizaje escolar y entrenamiento cognitivo se muestran por separado y no se combinan en una nota única.</footer></main></body></html>'
  }
  ...
  function placeFamilyIntegralBlock(card,block){
    ...
    var profile=card.querySelector(":scope > .cocoFamilyProfileV160100");
    if(profile){var afterProfile=profile.nextSibling;if(afterProfile!==block)card.insertBefore(block,afterProfile);return true}
    ...
    var firstContent=Array.prototype.slice.call(card.children||[]).find(function(n){return n!==block&&!(n.matches&&n.matches(".eternaV159FamilyStatus,.eternaV160FamilyPromo,.eternaV160ProgressPanel,.eternaV159ParentSettings,.eternaLegalV16058,.cocoFamilyProfileV160100,[data-coco-family-data-source='games']"))});
  ...
  function refreshFamilyAfterProfileChange(event){
    var card=event&&event.detail&&event.detail.card||familyCardNode(),modal=card&&card.closest?card.closest(".cocoFamilyV129"):familyModalNode();
    if(!modal||!card)return;
    familyReportState.games=null;familyReportState.gamesStatus="idle";
    renderFamilyIntegral(modal,card,{skipLearningLoad:true})
  }
  root.addEventListener("coco:family-profile-ready",refreshFamilyAfterProfileChange,{passive:true});
  root.addEventListener("coco:family-profile-updated",refreshFamilyAfterProfileChange,{passive:true});
  ...
  function makePublicMiniAccessible(card){
    if(!card||!card.matches||!card.matches(".cocoMiniJuego[data-coco-juego]"))return false;
    if(card.tagName==="BUTTON")return true;
    var title=card.querySelector("b"),name=String(title&&title.textContent||"este juego").trim();
    card.setAttribute("role","button");card.setAttribute("tabindex","0");card.setAttribute("aria-label","Inicia sesión para abrir "+name);
    if(card.dataset.cocoKeyboardAccess!=="1"){card.dataset.cocoKeyboardAccess="1";card.addEventListener("keydown",function(event){if(event.key!=="Enter"&&event.key!==" ")return;event.preventDefault();card.click()})}
    return true
  }
  function processNode(node){
    ...
    if(node.matches&&node.matches(".cocoMiniJuego[data-coco-juego]"))makePublicMiniAccessible(node);
    if(node.matches&&node.matches(".cocoGameCard[data-coco-juego]"))applyCard(node);
    if(node.querySelectorAll&&hasCatalog){node.querySelectorAll("[data-coco-juego]").forEach(function(n){if(RETIRED.has(idOf(n)))n.remove()});node.querySelectorAll(".cocoMiniJuego[data-coco-juego]").forEach(makePublicMiniAccessible);node.querySelectorAll(".cocoGameCard[data-coco-juego]").forEach(applyCard)}
  ...
  function loadEternaExperience(intent){
    ...
    eternaPromise=new Promise(function(resolve,reject){
      ...
      s.src="./eterna-experience-v160.js?v=160100";
      s.async=true;
      s.dataset.cocoModule="eterna-experience-v160100";
      s.onload=function(){
  ...
  function flush(){
    raf=0;var nodes=Array.from(queued);queued.clear();nodes.forEach(processNode);if(!trueFalseMixInstalled)installTrueFalseMixGuard();if(catalogDirty)finalizeCatalog(document.getElementById("cocoApp"));root.COCO_VERSION="2026-09-05-v160.94.2-audit-1-3"
  }
  ...
  function initial(){
    ...
    if(app){Array.prototype.slice.call(app.querySelectorAll(".cocoMiniJuego[data-coco-juego]")).forEach(makePublicMiniAccessible);Array.prototype.slice.call(app.querySelectorAll(".cocoGameCard[data-coco-juego]")).forEach(applyCard);ensurePublicFreemiumMessage(app);finalizeCatalog(app);var openFamily=app.querySelector(".cocoFamilyV129");if(openFamily)polishFamilyRuntime(openFamily)}
    scheduleEternaIdle();root.COCO_VERSION="2026-09-05-v160.94.2-audit-1-3"
  }
