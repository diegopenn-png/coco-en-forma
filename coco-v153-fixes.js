...
(function(root){
  ...
  function buildGamesReportModel(modal,card){
    ...
    var bars=Array.prototype.slice.call(barRoot.querySelectorAll(":scope > article,article")).map(function(x){var b=x.querySelector("b"),sp=x.querySelector("span"),strong=x.querySelector("strong");return{label:b?String(b.textContent||"").trim():"Capacidad",sublabel:sp?String(sp.textContent||"").trim():"",value:strong?numberFromText(strong.textContent):0}}).filter(function(x){return x.label&&!retiredEnglishCopy(x.label)&&!retiredEnglishCopy(x.sublabel)});
    var groups=coverage?Array.prototype.slice.call(coverage.querySelectorAll(":scope > article,article")).map(function(x){var b=x.querySelector("b"),items=Array.prototype.slice.call(x.querySelectorAll("span")).map(function(s){return String(s.textContent||"").trim()}).filter(function(item){return item&&!retiredEnglishCopy(item)});return{title:b?String(b.textContent||"").trim():"Área",items:items}}).filter(function(group){return !retiredEnglishCopy(group.title)}):[];
  ...
  function loadEternaExperience(intent){
    ...
    eternaPromise=new Promise(function(resolve,reject){
      ...
      s.src="./eterna-experience-v160.js?v=1609410";
      s.async=true;
      s.dataset.cocoModule="eterna-experience-v1609410";
      s.onload=function(){
