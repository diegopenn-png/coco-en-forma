/* Coco en Forma · v160 FINAL4.22 · CTA único + onboarding trial */
(function(root){
  "use strict";
  ...
  function loadOnboardingPatch(){
    if(root.__ETERNA_ONBOARDING_V16062_LOADER__)return;root.__ETERNA_ONBOARDING_V16062_LOADER__=true;
    var p=document.createElement("script");p.src="./eterna-onboarding-v16062.js?v=16062";p.async=true;p.dataset.cocoModule="eterna-onboarding-v16062";
    p.onerror=function(){root.__ETERNA_ONBOARDING_V16062_LOADER__=false};document.head.appendChild(p)
  }
  function loadFamilyPatch(){
    if(root.__ETERNA_FAMILY_V16061_LOADER__){loadOnboardingPatch();return}
    root.__ETERNA_FAMILY_V16061_LOADER__=true;
    var p=document.createElement("script");p.src="./eterna-family-v16061.js?v=16061";p.async=true;p.dataset.cocoModule="eterna-family-v16061";
    p.onload=loadOnboardingPatch;p.onerror=function(){root.__ETERNA_FAMILY_V16061_LOADER__=false;loadOnboardingPatch()};document.head.appendChild(p)
  }
  function loadEternaExperience(){
    if(root.__ETERNA_EXPERIENCE_V16062_LOADER__)return;root.__ETERNA_EXPERIENCE_V16062_LOADER__=true;
    ...
    s.onload=loadFamilyPatch;s.onerror=function(){root.__ETERNA_EXPERIENCE_V16062_LOADER__=false};document.head.appendChild(s)
  }
  function flush(){raf=0;var nodes=Array.from(queued);queued.clear();nodes.forEach(processNode);root.COCO_VERSION="2026-08-24-v160-final4.22"}
  function queue(node){if(node)queued.add(node);if(!raf)raf=requestAnimationFrame(flush)}
  function initial(){var app=document.getElementById("cocoApp");if(app)processNode(app);loadEternaExperience();root.COCO_VERSION="2026-08-24-v160-final4.22"}
