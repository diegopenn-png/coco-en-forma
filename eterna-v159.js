...
(function(){
  ...
  async function speak(text,rate,button,voiceDialog){if(state.parentSettings&&state.parentSettings.voice_enabled===false){announceVoiceState("idle");alert("La voz de Eterna está desactivada desde Zona familiar.");return}if(button&&state.lastSpeechButton===button&&state.lastAudio){if(state.lastAudio.paused){await state.lastAudio.play();speechButtonLabel(button,true);announceVoiceState("speaking")}else{state.lastAudio.pause();speechButtonLabel(button,false);announceVoiceState("idle")}return}stopAudio();state.lastSpeechButton=button||null;speechButtonLabel(button,"loading");announceVoiceState("preparing");try{var r=await api("/v1/speak",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:cleanText(String(text||"")).slice(0,1800)})});if(!r.ok)throw new Error("TTS");var blob=await r.blob();if(state.lastSpeechUrl)URL.revokeObjectURL(state.lastSpeechUrl);state.lastSpeechUrl=URL.createObjectURL(blob);var a=voiceDialog&&window.__ETERNA_VOICE_AUDIO__?window.__ETERNA_VOICE_AUDIO__:new Audio();a.src=state.lastSpeechUrl;a.muted=false;a.volume=1;a.playbackRate=rate||1;state.lastAudio=a;a.onended=function(){speechButtonLabel(button,false);announceVoiceState("idle");if(state.lastAudio===a){state.lastAudio=null;state.lastSpeechButton=null}};a.onerror=a.onended;speechButtonLabel(button,true);announceVoiceState("speaking");await a.play();return}catch(e){speechButtonLabel(button,false);state.lastSpeechButton=null;announceVoiceState("idle");setStatus("No pude preparar la voz de Eterna. Inténtalo de nuevo.","warn")}}
