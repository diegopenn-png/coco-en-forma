...
(function(){
  ...
  var VERSION="160.99.25-contextual-dialogue";
  ...
  var GREETING_KEY_PREFIX="coco_eterna_greeting_v1:";
  ...
  function greetingPeriodLabel(period){return period==="morning"?"Buenos días":period==="afternoon"?"Buenas tardes":"Buenas noches"}
  function greetingStorageKey(){var uid=sessionUserId();return uid?GREETING_KEY_PREFIX+uid:""}
  function clientGreetingState(){var key=greetingStorageKey();if(!key)return null;try{var value=JSON.parse(localStorage.getItem(key)||"null");if(!value||!/^(?:\d{4})-(?:\d{2})-(?:\d{2})$/.test(String(value.local_date||""))||["morning","afternoon","night"].indexOf(value.period)<0||!Number.isFinite(Number(value.greeted_at_ms)))return null;return{local_date:String(value.local_date),period:String(value.period),greeted_at_ms:Number(value.greeted_at_ms)}}catch(e){return null}}
  function saveGreetingState(value){var key=greetingStorageKey();if(!key||!value||typeof value!=="object")return;try{localStorage.setItem(key,JSON.stringify({local_date:String(value.local_date||""),period:String(value.period||""),greeted_at_ms:Number(value.greeted_at_ms||Date.now())}))}catch(e){}}
  function clientClock(){var now=new Date();return{local_date:localDateKey(now),local_hour:now.getHours(),timezone_offset_minutes:now.getTimezoneOffset(),sent_at_ms:now.getTime()}}
  function openingGreeting(studentName){var clock=clientClock(),period=greetingPeriod(clock.local_hour),previous=clientGreetingState();if(previous&&previous.local_date===clock.local_date&&previous.period===period)return"";saveGreetingState({local_date:clock.local_date,period:period,greeted_at_ms:clock.sent_at_ms});return greetingPeriodLabel(period)+(studentName?", "+studentName:"")+". "}
  ...
  function invalidateInFlight(reason){state.activityEpoch+=1;if(state.activeRequest&&state.activeRequest.controller){try{state.activeRequest.controller.abort(reason||"activity-invalidated")}catch(e){}}state.activeRequest=null;state.busy=false;setThinking(false);var o=document.getElementById("eternaOverlayV159");if(o){var input=o.querySelector("[data-et-input]"),button=o.querySelector("[data-et-send]");if(input)input.disabled=false;if(button)button.disabled=false}try{window.dispatchEvent(new CustomEvent("coco:eterna-context-invalidated",{detail:{reason:reason||"activity-invalidated",mode:state.mode}}))}catch(e){}}
  ...
  function renderModeBar(){
    ...
    if(state.mode==="exam"&&Number(ms.question_number||0)>0)progress='<span class="eternaV160ModeProgress">Pregunta '+esc(ms.question_number)+' · Aciertos '+esc(ms.correct_count||0)+' · Por revisar '+esc((ms.partial_count||0)+(ms.incorrect_count||0))+'</span>';
    else if(state.mode==="practice"&&(Number(ms.question_number||0)>0||ms.focus))progress='<span class="eternaV160ModeProgress">Ejercicio '+esc(ms.question_number||0)+' · Aciertos '+esc(ms.correct_count||0)+' · Errores '+esc((ms.partial_count||0)+(ms.incorrect_count||0))+(ms.focus?' · Reforzando '+esc(ms.focus):'')+'</span>';
    if(bar){bar.innerHTML='<span class="eternaV160ModeIcon">'+m.icon+'</span><span><b>Modo: '+esc(m.label)+'</b><small>'+esc(m.description)+'</small>'+progress+'</span><span class="eternaV160ModeActions"><button type="button" class="eternaV160NewActivity" data-et-newactivity>Nueva actividad</button><button type="button" class="eternaV160ChangeMode" data-et-changemode>Cambiar modo</button><button type="button" class="eternaV160Conversation" data-et-converse aria-label="Iniciar conversación hablada con Eterna"><span class="eternaV160ConversationIcon" aria-hidden="true"><svg viewBox="0 0 56 48" focusable="false"><path d="M7 15h23a7 7 0 0 1 7 7v8a7 7 0 0 1-7 7H18l-8 7v-7H7a7 7 0 0 1-7-7v-8a7 7 0 0 1 7-7Z"></path><path d="M22 3h25a7 7 0 0 1 7 7v9a7 7 0 0 1-7 7h-3v7l-8-7H22a7 7 0 0 1-7-7v-9a7 7 0 0 1 7-7Z"></path><circle cx="27" cy="14.5" r="1.7"></circle><circle cx="34.5" cy="14.5" r="1.7"></circle><circle cx="42" cy="14.5" r="1.7"></circle></svg></span><span class="eternaV160ConversationCopy"><b data-et-converse-title>Conversar con Eterna</b><small data-et-converse-copy>Habla y Eterna te responde con su voz</small></span></button></span>';bar.querySelector("[data-et-changemode]").onclick=showModePicker;bar.querySelector("[data-et-newactivity]").onclick=function(){closeActivity(state.mode);resetVisibleSession(true);renderModeBar();setStatus("Nueva actividad · "+m.label,"ok");var i=o.querySelector("[data-et-input]");if(i)i.focus()}}
    syncModeButtons()
  }
  ...
  function setThinking(active){var o=document.getElementById("eternaOverlayV159");if(!o)return;var thinking=o.querySelector("[data-et-thinking]"),chat=o.querySelector("[data-et-chat]"),isActive=active===true;if(thinking){thinking.hidden=!isActive;thinking.classList.toggle("is-visible",isActive);thinking.setAttribute("aria-hidden",isActive?"false":"true")}if(chat)chat.setAttribute("aria-busy",isActive?"true":"false")}
  ...
  function understoodActionUseful(meta,text){if(!meta||meta.show_understood!==true||meta.check_question||!["ask","explain"].includes(state.mode))return false;var value=cleanText(text||""),words=value.split(/\s+/).filter(Boolean);return words.length>=8&&!/\?\s*$/.test(value)}
  ...
  function applyChatResponse(data,context){
    ...
    var previousPoints=state.pedagogicalState&&Array.isArray(state.pedagogicalState.explained_points)?state.pedagogicalState.explained_points:[],nextPedagogical=data.pedagogical_state&&typeof data.pedagogical_state==="object"?data.pedagogical_state:null,nextPoints=nextPedagogical&&Array.isArray(nextPedagogical.explained_points)?nextPedagogical.explained_points:[],hasNewPoint=nextPoints.some(function(point){return previousPoints.indexOf(point)<0});
    var meta={verification_status:data.verification_status||"needs_clarification",subject:cleanMetaText(data.subject),concept:cleanMetaText(data.concept),help_level:data.help_level,check_question:data.check_question||null,practice_suggestion:data.practice_suggestion||null,student_answer_assessment:data.student_answer_assessment||"not_applicable",strategy_used:data.strategy_used||null,mode_label:data.mode_label||null,event_id:data.event_id||key||null,session_id:valid.state.session_id,question_id:valid.state.question_id,request_id:context.request_id,client_turn_id:context.client_turn_id,show_understood:Boolean(!data.check_question&&nextPedagogical&&nextPedagogical.last_tutor_act==="explain"&&hasNewPoint)};
    ...
    state.history.push({role:"assistant",text:reply,meta:meta});state.lastReply=reply;persistLearningSession();var voiceDialog=window.__ETERNA_VOICE_DIALOG_ACTIVE__===true;renderModeBar();appendMessage("assistant",reply,meta,true,true);if(voiceDialog||(data.verification_status==="verified"&&data.auto_speak===true))speak(reply,1,null,voiceDialog);if(voiceDialog){window.__ETERNA_SUPPRESS_SYNTHETIC_LISTEN_UNTIL__=Date.now()+5000;window.__ETERNA_VOICE_DIALOG_ACTIVE__=false}clearImage();state.inputSource="text";state.dataLoadedAt=0;setResultStatus(data);try{window.dispatchEvent(new CustomEvent("coco:eterna-response-applied",{detail:{request_id:context.request_id,client_turn_id:context.client_turn_id,session_id:context.session_id,mode:context.mode}}))}catch(e){}return{applied:true,activity_state:valid.state,meta:meta}
  }
  ...
  function announceVoiceState(value){try{window.dispatchEvent(new CustomEvent("coco:eterna-voice-state",{detail:{state:value||"idle"}}))}catch(e){}}
  ...
  async function toggleRecord(){
    ...
    if(state.parentSettings&&state.parentSettings.allow_audio_input===false){announceVoiceState("idle");alert("El micrófono está desactivado desde Zona familiar.");return}
    if(state.recorder&&state.recorder.state==="recording"){announceVoiceState("transcribing");state.recorder.stop();return}
    if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||typeof MediaRecorder==="undefined"){announceVoiceState("idle");alert("Este navegador no permite grabar audio desde Eterna. Puedes escribir la pregunta.");return}
    try{
      var permission=await microphonePermissionState();if(permission==="denied"){announceVoiceState("idle");alert("El micrófono está bloqueado para Coco en Forma. Actívalo en los permisos del navegador o de la app.");return}
      ...
      rec.onstop=async function(){b.classList.remove("recording");b.textContent="🎙️";stream.getTracks().forEach(function(t){t.stop()});announceVoiceState("transcribing");var type=rec.mimeType||mime||(state.chunks[0]&&state.chunks[0].type)||"audio/webm",blob=new Blob(state.chunks,{type:type});state.recorder=null;await transcribe(blob,audioFilename(type))};
      rec.start(250);b.classList.add("recording");b.textContent="■";announceVoiceState("listening");setStatus("Escuchando… toca de nuevo para terminar","warn")
    }catch(e){announceVoiceState("idle");window.__ETERNA_VOICE_DIALOG_ACTIVE__=false;alert("No se pudo acceder al micrófono. Revisa el permiso del navegador.")}
  }

  async function transcribe(blob,name){var voiceDialog=window.__ETERNA_VOICE_DIALOG_ACTIVE__===true;setStatus("Transcribiendo tu pregunta…","warn");try{var fd=new FormData();fd.append("audio",blob,name||audioFilename(blob.type));var r=await api("/v1/transcribe",{method:"POST",body:fd}),data=await safeJson(r);if(!r.ok||!data.text)throw new Error("TRANSCRIPTION_FAILED");var i=overlay().querySelector("[data-et-input]");i.value=cleanText(data.text);state.inputSource="voice";if(voiceDialog){announceVoiceState("thinking");setStatus("Te he oído. Eterna está preparando su respuesta…","warn");await send()}else{announceVoiceState("idle");setStatus("He escrito lo que te he oído. Revísalo y envíalo.","ok");i.focus()}}catch(e){window.__ETERNA_VOICE_DIALOG_ACTIVE__=false;announceVoiceState("idle");setStatus("No pude transcribir el audio. Puedes escribirlo.","warn")}}
  ...
  async function fetchSpeechBlob(text){var lastError=null,payload=JSON.stringify({text:cleanText(String(text||"")).slice(0,1800)});for(var attempt=0;attempt<2;attempt++){var controller=typeof AbortController!=="undefined"?new AbortController():null,timer=controller?setTimeout(function(){controller.abort()},15000):null;try{var options={method:"POST",headers:{"Content-Type":"application/json"},body:payload};if(controller)options.signal=controller.signal;var r=await api("/v1/speak",options);if(r.ok)return await r.blob();lastError=new Error("TTS_"+r.status)}catch(e){lastError=e}finally{if(timer)clearTimeout(timer)}if(attempt===0)setStatus("La voz tarda un poco. Reintentando…","warn")}throw lastError||new Error("TTS")}
  async function speak(text,rate,button,voiceDialog){if(state.parentSettings&&state.parentSettings.voice_enabled===false){announceVoiceState("idle");alert("La voz de Eterna está desactivada desde Zona familiar.");return}if(button&&state.lastSpeechButton===button&&state.lastAudio){if(state.lastAudio.paused){await state.lastAudio.play();speechButtonLabel(button,true);announceVoiceState("speaking")}else{state.lastAudio.pause();speechButtonLabel(button,false);announceVoiceState("idle")}return}stopAudio();state.lastSpeechButton=button||null;speechButtonLabel(button,"loading");announceVoiceState("preparing");try{var blob=await fetchSpeechBlob(text);if(state.lastSpeechUrl)URL.revokeObjectURL(state.lastSpeechUrl);state.lastSpeechUrl=URL.createObjectURL(blob);var a=voiceDialog&&window.__ETERNA_VOICE_AUDIO__?window.__ETERNA_VOICE_AUDIO__:new Audio();a.src=state.lastSpeechUrl;a.muted=false;a.volume=1;a.playbackRate=rate||1;state.lastAudio=a;a.onended=function(){speechButtonLabel(button,false);announceVoiceState("idle");if(state.lastAudio===a){state.lastAudio=null;state.lastSpeechButton=null}};a.onerror=a.onended;speechButtonLabel(button,true);announceVoiceState("speaking");await a.play();return}catch(e){speechButtonLabel(button,false);state.lastSpeechButton=null;announceVoiceState("idle");setStatus("No pude preparar la voz de Eterna. Toca Escuchar para intentarlo de nuevo.","warn")}}
  ...
  function buildFamilyLearningReportModel(exportData){
    ...
    var academicMemory=(Array.isArray(exportData.academic_memory)?exportData.academic_memory:[]).filter(function(x){return x&&cleanText(x.topic_label)}).slice(0,12);
    var concepts=(Array.isArray(exportData.student_concept_memory)?exportData.student_concept_memory:[]).filter(function(x){return x&&cleanText(x.concept_label)});
    var observed=concepts.filter(function(x){return Number(x.attempts||0)>0}),evaluable=observed.filter(function(x){return Number(x.attempts||0)>=2});
    var strongest=evaluable.filter(function(x){return Number(x.mastery_score||0)>=.65}).sort(function(a,b){return Number(b.mastery_score||0)-Number(a.mastery_score||0)}).slice(0,3);
    var reinforce=evaluable.filter(function(x){return Number(x.mastery_score||0)<.65}).sort(function(a,b){return Number(a.mastery_score||0)-Number(b.mastery_score||0)}).slice(0,3);
    var strategies=(Array.isArray(exportData.learning_strategy_memory)?exportData.learning_strategy_memory:[]).filter(function(x){return x&&cleanText(x.strategy_key)&&Number(x.evidence_count||0)>=2}).sort(function(a,b){var ds=Number(b.success_score||0)-Number(a.success_score||0);return Math.abs(ds)>.001?ds:Number(b.evidence_count||0)-Number(a.evidence_count||0)}).slice(0,3);
    var profile=exportData.student_profile||state.profile||{},subjects=familyUniqueStrings(observed.map(function(x){return x.subject}).concat(academicMemory.map(function(x){return x.subject}))),attempts=observed.reduce(function(sum,x){return sum+Number(x.attempts||0)},0),errors=observed.reduce(function(sum,x){return sum+Number(x.errors||0)},0),partials=observed.reduce(function(sum,x){return sum+Number(x.partials||0)},0),independent=observed.reduce(function(sum,x){return sum+Number(x.independent_successes||0)},0),assisted=observed.reduce(function(sum,x){return sum+Number(x.assisted_successes||0)},0),activeDays=familyRecentUsageDays(exportData.usage||[]);
    var lead=evaluable.slice().sort(function(a,b){return Number(b.mastery_score||0)-Number(a.mastery_score||0)})[0]||null,next=reinforce[0]||null,subjectCopy=subjects.join(", ");
    var summary=!subjects.length?"Todavía no hay actividad escolar suficiente para preparar una valoración.":!evaluable.length?"Ha trabajado "+subjectCopy+", pero todavía necesitamos más de una interacción sobre un mismo concepto antes de sacar conclusiones.":"Ha trabajado "+subjectCopy+". Solo valoramos los conceptos con al menos dos interacciones; el resto se muestra como actividad, sin inventar conclusiones.";
    var bars=evaluable.slice().sort(function(a,b){return Number(b.attempts||0)-Number(a.attempts||0)}).slice(0,6).map(function(x){return{label:cleanText(x.concept_label),sublabel:cleanText(x.subject)||"Aprendizaje",value:percent(x.mastery_score),detail:familyCountLabel(x.attempts,"interacción","interacciones")}}),panels=[];
    if(strongest.length)panels.push({tone:"strength",icon:"★",eyebrow:"LO QUE MEJOR ENTIENDE",title:"Aprendizajes más asentados",text:"Solo aparecen cuando ya hay práctica repetida.",items:strongest.map(function(x){return{label:cleanText(x.concept_label),detail:cleanText(x.subject)||"Aprendizaje",percent:percent(x.mastery_score)}})});
    if(reinforce.length)panels.push({tone:"reinforce",icon:"↗",eyebrow:"PARA SEGUIR PRACTICANDO",title:"Aprendizajes todavía en proceso",text:"No son etiquetas: pueden cambiar con la práctica.",items:reinforce.map(function(x){return{label:cleanText(x.concept_label),detail:(cleanText(x.subject)||"Aprendizaje")+" · "+familyCountLabel(x.attempts,"interacción","interacciones"),percent:percent(x.mastery_score)}})});
    if(strategies.length)panels.push({tone:"strategy",icon:"💡",eyebrow:"CÓMO LE AYUDA ETERNA",title:"Formas de explicar que le están ayudando",text:"Solo se muestran estrategias observadas más de una vez.",items:strategies.map(function(x){var score=Number(x.success_score);return{label:strategyName(x.strategy_key),detail:familyCountLabel(x.evidence_count,"observación","observaciones"),percent:isFinite(score)?percent(score):null}})});
    if(attempts)panels.push({tone:"activity",icon:"◎",eyebrow:"ACTIVIDAD",title:"Qué ha ocurrido en las actividades",items:[{label:"Respuestas correctas con poca ayuda",value:independent},{label:"Respuestas correctas con apoyo",value:assisted},{label:"Respuestas por revisar",value:errors},{label:"Respuestas parcialmente correctas",value:partials}]});
    return{academicMemory:academicMemory,theme:"learning",eyebrow:"APRENDIZAJE · ETERNA",title:"Lo que ha trabajado con Eterna",subtitle:"Un resumen sencillo de las materias y conceptos que realmente ha practicado.",summary:summary,personName:familyLearningSupportName(),personMeta:[profile.school_year||"Curso no indicado",profile.autonomous_community||""].filter(Boolean).join(" · "),hero:{eyebrow:lead?"MAYOR AVANCE OBSERVADO":"SIN VALORACIÓN TODAVÍA",title:lead?"Más avance en: "+cleanText(lead.concept_label):"Necesitamos un poco más de práctica",text:lead?"Esta valoración se basa en práctica repetida y puede cambiar con nuevas actividades.":"Una sola interacción no basta para decir qué entiende mejor o qué necesita reforzar.",percent:lead?percent(lead.mastery_score):null},signalDefinition:FAMILY_SIGNAL_DEFINITION,metrics:[{value:subjects.length,label:"materias trabajadas"},{value:attempts,label:"interacciones registradas"},{value:evaluable.length,label:"conceptos con práctica suficiente"},{value:activeDays+"/7",label:"días activos esta semana"}],bars:bars,barEyebrow:"CON PRÁCTICA SUFICIENTE",barTitle:"Avance por conceptos",barScale:"Avance orientativo · 0–100",panels:panels,groups:subjects.length?[{title:"Materias trabajadas",items:subjects}]:[],groupEyebrow:"MATERIAS",groupTitle:"Dónde ha trabajado",nextStep:{eyebrow:"QUÉ RECOMENDAMOS AHORA",title:next?"Practicar: "+cleanText(next.concept_label):evaluable.length?"Seguir practicando las materias trabajadas":"Repetir algún concepto en otra actividad",text:next?"Conviene volver a este concepto con pasos cortos y una comprobación al final.":evaluable.length?"Mantener una práctica variada ayudará a confirmar y ampliar estos avances.":"Con una segunda actividad sobre el mismo concepto podremos empezar a valorar el progreso con más fundamento."},note:"Informe pedagógico y orientativo. Analiza solo la actividad escolar registrada; no es una nota, una etiqueta ni un diagnóstico."}
  }
