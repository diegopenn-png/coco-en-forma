...
(function(){
  ...
  function renderModeBar(){
    ...
    if(bar){bar.innerHTML='<span class="eternaV160ModeIcon">'+m.icon+'</span><span><b>Modo: '+esc(m.label)+'</b><small>'+esc(m.description)+'</small>'+progress+'</span><span class="eternaV160ModeActions"><button type="button" class="eternaV160NewActivity" data-et-newactivity>Nueva actividad</button><button type="button" class="eternaV160ChangeMode" data-et-changemode>Cambiar modo</button><button type="button" class="eternaV160Conversation" data-et-converse aria-label="Conversar por voz con Eterna">🎙️ Conversar con Eterna</button></span>';bar.querySelector("[data-et-changemode]").onclick=showModePicker;bar.querySelector("[data-et-newactivity]").onclick=function(){closeActivity(state.mode);resetVisibleSession(true);renderModeBar();setStatus("Nueva actividad · "+m.label,"ok");var i=o.querySelector("[data-et-input]");if(i)i.focus()}}
    syncModeButtons()
  }
  ...
  async function speak(text,rate,button){if(state.parentSettings&&state.parentSettings.voice_enabled===false){alert("La voz de Eterna está desactivada desde Zona familiar.");return}if(button&&state.lastSpeechButton===button&&state.lastAudio){if(state.lastAudio.paused){await state.lastAudio.play();speechButtonLabel(button,true)}else{state.lastAudio.pause();speechButtonLabel(button,false)}return}stopAudio();state.lastSpeechButton=button||null;speechButtonLabel(button,"loading");try{var r=await api("/v1/speak",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:cleanText(String(text||"")).slice(0,1800)})});if(!r.ok)throw new Error("TTS");var blob=await r.blob();if(state.lastSpeechUrl)URL.revokeObjectURL(state.lastSpeechUrl);state.lastSpeechUrl=URL.createObjectURL(blob);var a=new Audio(state.lastSpeechUrl);a.playbackRate=rate||1;state.lastAudio=a;a.onended=function(){speechButtonLabel(button,false);if(state.lastAudio===a){state.lastAudio=null;state.lastSpeechButton=null}};a.onerror=a.onended;speechButtonLabel(button,true);await a.play();return}catch(e){speechButtonLabel(button,false);state.lastSpeechButton=null;setStatus("No pude preparar la voz de Eterna. Inténtalo de nuevo.","warn")}}
