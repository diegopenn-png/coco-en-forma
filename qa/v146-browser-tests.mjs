...
await test("Browser: Coco Corre muestra objetivo real, sin revelar respuestas y con Coco estable", async () => {
  ...
  await page.evaluate(() => { window.__qaRunnerWrites = 0; window.__qaRunnerLast = null; window.CocoArcadeV133.saveScore = async (...args) => { window.__qaRunnerWrites += 1; window.__qaRunnerLast = args; return { ok: true, stats: { today: 1, total: args[1] } }; }; });
  ...
  const audit = await page.evaluate(() => window.CocoRunnerV146.audit());
  ...
  const triangle = await page.evaluate(() => {
    const runner = window.CocoRunnerV146, shape = runner.catalog().shapes.find(item => item.id === "triangulo"), random = () => .37;
...
await test("Browser: Encuentra las diferencias completa todos los escenarios y niveles desde ambas imágenes", async () => {
  ...
  const audit = await diffPage.evaluate(() => window.CocoDifferencesProV146.audit()); assert.deepEqual(audit.differenceKinds, ["color", "shape", "presence"]); assert.equal(audit.brokenObjects, false); assert.equal(audit.deformedObjects, false);
...
await test("Browser: service worker v146, sin errores JavaScript ni 404 locales", async () => {
  const cacheVersion = await page.evaluate(async () => { if (!("caches" in window)) return []; return caches.keys(); }); assert.ok(cacheVersion.some(key => key.includes("v146.0.0")) || cacheVersion.length === 0);
