...
test("Coco Pádel: exactamente Mixing, Campeonato y Jugadores", () => {
  const padel = evaluate("coco-v144-padel.js").CocoPadelV147;
...
test("Coco Pádel: nombres duplicados conservan códigos únicos y no reutilizables", () => {
  const padel = evaluate("coco-v144-padel.js").CocoPadelV147, state = padel.blankState();
...
test("Runner: todas las misiones pasan validación previa en los tres niveles", () => {
  const runner = evaluate("coco-v144-runner.js").CocoRunnerV147;
...
test("Runner: triángulos, estrellas, círculos, cuadrados y todas las reglas tienen objetivo y distractor", () => {
  const runner = evaluate("coco-v144-runner.js").CocoRunnerV147, catalog = runner.catalog();
...
test("Runner: finito, una vez al día y en la clasificación general", () => {
  const runner = evaluate("coco-v144-runner.js").CocoRunnerV147, audit = runner.audit();
...
test("Encuentra las diferencias: cambios integrados y ninguna pista previa", () => {
  const differences = evaluate("coco-v144-differences.js").CocoDifferencesProV147, audit = differences.audit();
...
test("PWA: caché v147 contiene todos sus recursos y no contiene Inglés", () => {
  const sw = fs.readFileSync(path.join(rootDir, "sw.js"), "utf8");
  assert.match(sw, /coco-en-forma-v147\.0\.0/); assert.match(sw, /coco-v147-refinements\.css/); assert.doesNotMatch(sw, /ingles|english/i);
...
test("Integración: v147 carga los módulos corregidos y registra Coco Corre como general", () => {
  const html = fs.readFileSync(path.join(rootDir, "index.html"), "utf8");
  for (const file of ["coco-v144-core.js?v=14700", "coco-v144-padel.js?v=14700", "coco-v144-runner.js?v=14700", "coco-v147-refinements.css?v=14700"]) assert.ok(html.includes(file), file);
  assert.ok(html.includes("2026-08-15-v147.0-profesional")); assert.ok(html.includes('id:"cococorre",nombre:"Coco Corre",grupo:"general"'));
