...
const differences = window.CocoDifferencesProV147;
  ...
  for (let variant = 0; variant < 3; variant += 1) {
    for (const level of [1, 2, 3]) {
      ...
      for (const item of items) {
        ...
        if (item.kind === "presence" || item.kind === "color") {
          assert.equal(leftFromBase, 0, `${item.id}: el lado sin objeto debe conservar la escena natural`);
          assert.ok(rightFromBase > 100, `${item.id}: el cambio debe ser visible e integrado`);
        } else {
...
assert.equal(audit.preAnswerMarkers, false); assert.equal(audit.genericCircleMarkers, false); assert.equal(audit.genericStarMarkers, false);
console.log("PASS  Solo se renderizan cambios integrados de color, forma completa y presencia/ausencia; no hay círculos ni estrellas que señalen respuestas.");
...
if (evidenceRows.length) {
  ...
  const evidenceDir = path.join(qaDir, "evidence-v147"); fs.mkdirSync(evidenceDir, { recursive: true });
  const evidencePath = path.join(evidenceDir, "encuentra-las-diferencias-v147.jpg");
