...
import http.server,threading,shutil,json
...
with sync_playwright() as p:
 ...
 page.wait_for_function("async()=>{const ks=await caches.keys();return ks.includes('coco-en-forma-v160.99.4-reply-actions-r1')&&!ks.includes('coco-en-forma-v160.99.3-mic-pwa-capture-r1')}",timeout=15000)
 page.wait_for_function("async()=>(await (await fetch('/eterna-v159.js?v=160980')).text()).includes('Reply actions 160.99.4')",timeout=15000)
 fresh=page.evaluate("async()=>(await fetch('/eterna-v159.js?v=160980')).text()")
 assert 'Reply actions 160.99.4' in fresh and 'OLD_CORE_CACHE_FIXTURE' not in fresh
 ...
 report={'actual_service_worker_script':True,'initially_cached_old_core':True,'upgrade_installed':True,'old_cache_deleted':True,'old_asset_url_serves_updated_core':True,'removed_button_markup_absent':True,'microphone_and_speech_source_retained':True,'environment':'Chromium loopback; inert peripheral assets, no real user data','physical_iphone':False}
 o=ROOT/'reply-actions-live-evidence';o.mkdir(exist_ok=True);(o/'sw-update.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))
