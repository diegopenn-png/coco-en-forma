...
await test("Browser: Coco Corre muestra objetivo real, sin revelar respuestas y con Coco estable", async () => {
  ...
  const audit = await page.evaluate(() => window.CocoRunnerV148.audit());
  ...
  const triangle = await page.evaluate(() => {
    const runner = window.CocoRunnerV148, shape = runner.catalog().shapes.find(item => item.id === "triangulo"), random = () => .37;
...
await test("Browser: service worker v148, sin errores JavaScript ni 404 locales", async () => {
  const cacheVersion = await page.evaluate(async () => { if (!("caches" in window)) return []; return caches.keys(); }); assert.ok(cacheVersion.some(key => key.includes("v148.0.0")) || cacheVersion.length === 0);
