...
test("the PWA invalidates the human-teacher assets as one release", () => {
  ...
  assert.match(index, /sw\.js\?v=160114-r1/);
  assert.match(serviceWorker, /CACHE_VERSION="coco-en-forma-v160\.100\.14-reto-tiempo-advance-r1"/);
});
...
test("pending checks cannot be completed early and malformed replies stay retryable", () => {
  const client = read("eterna-v159.js");
  const worker = read("eterna-worker/src/index.js");
  ...
  assert.match(worker, /reply:\{type:"string",minLength:1\}/);
  assert.match(worker, /ETERNA_TUTOR_EMPTY_REPLY/);
});

test("safeguarding keeps a transient confirmation without storing the disclosure", () => {
  const client = read("eterna-v159.js");
  const worker = read("eterna-worker/src/index.js");
  assert.match(client, /safety_follow_up:null/);
  assert.match(worker, /SAFETY_FOLLOW_UP_CATEGORIES/);
  assert.match(worker, /teacher-core-safety-follow-up-v1/);
  assert.match(worker, /text:"\[safety follow-up\]"/);
  assert.match(worker, /Antes de cambiar de tema necesito comprobar algo importante/);
});
