...
test("Eterna has one canonical Worker entrypoint", () => {
  ...
  assert.match(worker, /160\.97\.5-ai-budget-resilience/);
...
test("the 160.97.5 production gate verifies the resilient model route before release", () => {
  ...
  assert.match(production, /EXPECTED_VERSION: 160\.97\.5-ai-budget-resilience/);
