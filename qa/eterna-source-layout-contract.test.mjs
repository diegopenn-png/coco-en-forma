...
test("Eterna has one canonical Worker entrypoint", () => {
  ...
  assert.match(worker, /160\.99\.11-cloudflare-regional-arithmetic-ocr/);
  ...
  assert.match(preview, /grep -F '160\.99\.11-cloudflare-regional-arithmetic-ocr' eterna-worker\/src\/index\.js/);
  assert.match(preview, /payload\.version === "160\.99\.11-cloudflare-regional-arithmetic-ocr"/);
  assert.match(preview, /API 160\.99\.11-cloudflare-regional-arithmetic-ocr/);
...
test("the 160.99.11 production gate performs focused Cloudflare regional arithmetic OCR before release", () => {
  const production = readFileSync(".github/workflows/eterna-worker-production-160995.yml", "utf8");
  const worker = readFileSync("eterna-worker/src/index.js", "utf8");
  assert.match(production, /\.github\/release-eterna-1609911/);
  assert.match(production, /EXPECTED_VERSION: 160\.99\.11-cloudflare-regional-arithmetic-ocr/);
