...
test("Eterna has one canonical Worker entrypoint", () => {
  ...
  assert.match(worker, /160\.99\.12-fast-regional-arithmetic-ocr/);
  ...
  assert.match(preview, /grep -F '160\.99\.12-fast-regional-arithmetic-ocr' eterna-worker\/src\/index\.js/);
  assert.match(preview, /payload\.version === "160\.99\.12-fast-regional-arithmetic-ocr"/);
  assert.match(preview, /API 160\.99\.12-fast-regional-arithmetic-ocr/);
...
test("the 160.99.12 production gate bounds focused Cloudflare regional arithmetic OCR before release", () => {
  const production = readFileSync(".github/workflows/eterna-worker-production-160995.yml", "utf8");
  const worker = readFileSync("eterna-worker/src/index.js", "utf8");
  assert.match(production, /\.github\/release-eterna-1609912/);
  assert.match(production, /EXPECTED_VERSION: 160\.99\.12-fast-regional-arithmetic-ocr/);
...
test("Eterna coherence and voice contracts stay wired into the PWA", () => {
  ...
  assert.match(serviceWorker, /160\.100\.3-photo-regions-r1/);
