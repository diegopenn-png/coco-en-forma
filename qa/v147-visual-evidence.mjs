...
const evidence = path.resolve("qa/evidence-v147");
...
await runner.screenshot({ path: path.join(evidence, "coco-corre-intro-390-v147.png") });
...
await runner.screenshot({ path: path.join(evidence, "coco-corre-partida-390-v147.png") });
await runner.setViewportSize({ width: 1440, height: 900 }); await runner.waitForTimeout(450);
await runner.screenshot({ path: path.join(evidence, "coco-corre-partida-1440-v147.png") });
...
await padel.screenshot({ path: path.join(evidence, "coco-padel-mixing-390-v147.png") });
await padel.getByRole("button", { name: "Jugadores", exact: true }).click();
await padel.screenshot({ path: path.join(evidence, "coco-padel-jugadores-390-v147.png") });
await padel.setViewportSize({ width: 320, height: 844 });
await padel.screenshot({ path: path.join(evidence, "coco-padel-jugadores-320-v147.png") });
await padel.setViewportSize({ width: 390, height: 844 });
await padel.getByRole("button", { name: "Campeonato", exact: true }).click();
await padel.screenshot({ path: path.join(evidence, "coco-padel-campeonatos-390-v147.png") });
await padel.getByRole("button", { name: /Liga familiar/ }).click();
await padel.screenshot({ path: path.join(evidence, "coco-padel-detalle-390-v147.png") });
...
await differences.screenshot({ path: path.join(evidence, "diferencias-partida-390-v147.png") });
await differences.setViewportSize({ width: 1440, height: 900 }); await differences.waitForTimeout(250);
await differences.screenshot({ path: path.join(evidence, "diferencias-partida-1440-v147.png") });
await differences.goto(`${base}/?qa=1&juego=diferencias&qaScene=music-studio&qaVariant=0`, { waitUntil: "domcontentloaded" });
await differences.locator(".c144DiffIntro").waitFor(); await differences.locator('[data-diff144-level="3"]').click(); await differences.locator("[data-diff144-start]").click();
await differences.locator('[data-diff144-scene="right"][data-ready="true"]').waitFor({ timeout: 10000 });
await differences.screenshot({ path: path.join(evidence, "diferencias-musica-avanzado-1440-v147.png") });
await differencesContext.close();

await browser.close();
console.log("PASS  Evidencia visual v147 generada en qa/evidence-v147.");
