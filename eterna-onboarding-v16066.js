/* ETERNA v160.66 · onboarding lanzamiento: email + PIN + autorización + trial
 ...
 * - Tras crear la cuenta, informa claramente de que hay que confirmar el correo electrónico.
 * - La intención de prueba se conserva 24 h para permitir confirmar el email y volver.
 * - Después pide el PIN familiar una sola vez.
...
(function(root){
  "use strict";
  if(root.__ETERNA_ONBOARDING_V16066__)return;
  root.__ETERNA_ONBOARDING_V16066__=true;

  var VERSION="160.66-launch-onboarding";
  var INTENT_KEY="coco_eterna_trial_intent_v16062";
  var ATTR_KEY="coco_eterna_attribution_v16062";
  var INTENT_TTL=24*60*60*1000;
  var PIN_PASS_KEY="coco_eterna_pin_pass_v16066";
  ...
  function injectStyles(){
    if(document.getElementById("eterna-onboarding-v16066-css"))return;
    var s=document.createElement("style");s.id="eterna-onboarding-v16066-css";
    s.textContent=[
      ...
      "#cocoApp .eternaV16066Toast{position:fixed;left:50%;bottom:max(18px,env(safe-area-inset-bottom));z-index:2147483000;transform:translateX(-50%);width:min(92vw,520px);padding:12px 15px;border:1px solid #cfe4ed;border-radius:14px;background:#fff;color:#173f59;box-shadow:0 12px 38px rgba(8,47,73,.22);font-size:11px;font-weight:800;line-height:1.45;text-align:center}",
  ...
  function toast(text,ms){
    var old=document.querySelector(".eternaV16066Toast");if(old)old.remove();
    var n=document.createElement("div");n.className="eternaV16066Toast";n.setAttribute("role","status");n.textContent=text;
  ...
  function emailActivationCopy(){
    return "Cuenta creada. Te hemos enviado un correo electrónico para activar tu cuenta. Revisa tu bandeja de entrada y, si no lo encuentras, la carpeta de spam o correo no deseado. Pulsa el enlace de confirmación antes de continuar con Eterna."
  }

  function enhanceSignupSuccessNotice(){
    if(!getIntent())return false;
    var app=document.getElementById("cocoApp");if(!app)return false;
    var candidates=app.querySelectorAll("h1,h2,h3,h4,strong,b");
    var heading=null;
    for(var i=0;i<candidates.length;i++){
      if(/^muy\s+bien[!.]?$/i.test(clean(candidates[i].textContent))){heading=candidates[i];break}
    }
    if(!heading)return false;
    heading.textContent="Revisa tu correo para activar la cuenta";
    var box=heading.parentElement||app;
    if(!box.querySelector("[data-et-email-activation-v16066]")){
      var n=document.createElement("div");
      n.setAttribute("data-et-email-activation-v16066","");
      n.style.cssText="margin:12px 0;padding:14px 16px;border:1px solid #bfe2cf;border-radius:14px;background:#effaf3;color:#245d45;font-weight:800;font-size:12px;line-height:1.5";
      n.textContent=emailActivationCopy();
  ...
  function scheduleSignupSuccessNotice(){
    [350,800,1500,2600,4200].forEach(function(ms){setTimeout(enhanceSignupSuccessNotice,ms)})
  }

  function installSignupAlertUpgrade(){
    if(root.__ETERNA_SIGNUP_ALERT_V16066__)return;
    root.__ETERNA_SIGNUP_ALERT_V16066__=true;
    var original=typeof root.alert==="function"?root.alert.bind(root):null;
    if(!original)return;
    root.alert=function(message){
      if(getIntent()&&/^muy\s+bien[!.]?$/i.test(clean(message)))return original(emailActivationCopy());
  ...
  async function resumePendingTrial(){
    ...
    try{
      ...
      if(!emailVerified(session)){
        toast("Confirma primero el correo electrónico que te hemos enviado para activar la cuenta.",6200);
  ...
  function onClick(event){
    ...
    if(clicked&&/^crear\s+cuenta\s+y\s+entrar$/i.test(clean(clicked.textContent)))scheduleSignupSuccessNotice();

    var familyOpen=event.target&&event.target.closest?event.target.closest(".cocoFamiliaBtn,[data-family-enter]"):null;
    if(familyOpen)ensureFamilyLayer();
  ...
  function install(){
    ...
    document.addEventListener("submit",function(event){
      ...
      if(form&&getIntent()&&/crear\s+cuenta/i.test(clean(form.textContent)))scheduleSignupSuccessNotice()
    },true);
